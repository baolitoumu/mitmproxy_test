...
#         print('获取到的url参数+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++',res)
        ...
        user_agent = 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.22 \
                      Safari/537.36 SE 2.X MetaSr 1.0'
        headers = {'User-Agent': user_agent }
        start_urls='//s.2.taobao.com/list/waterfall/waterfall.htm?wp='+wp+'&_ksTS='+_ksTS+'&stype='+stype+'&catid='+catid+'&divisionId='+divisionId+'&st_trust='+st_trust+'&q='+q+'&ist='+ist
        start_urls = response.urljoin(start_urls)
        ...
        yield scrapy.Request(url=start_urls, headers=headers,meta={'item_lasturl':item,'catid':catid,'divisionId':divisionId}, method='GET', callback=self.parse_json)
        ...
        if json_pages == 1:
            ...
            for json_page in range(json_pages):
                if json_page == 101:
                    ...
                    start_urls='s.2.taobao.com/list/waterfall/waterfall.htm?wp='+wp+'&_ksTS='+_ksTS+'&stype='+stype+'&catid='+catid+'&divisionId='+divisionId+'&st_trust='+st_trust+'&q='+q+'&ist='+ist
                    yield scrapy.Request(url=start_urls,meta={'data':item}, method='GET', callback=self.parse_json_end)
